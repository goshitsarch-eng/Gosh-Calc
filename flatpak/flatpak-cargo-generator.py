...
def fetch_git_repo(git_url: str, commit: str) -> str:
    ...
    # there are no submodules in the repository. Some repos pin submodule
    # commits their remote refuses to serve; those are not cargo dependencies,
    # so tolerate the failure.
    try:
        subprocess.run(
        ...
        logging.warning("Submodule update failed for %s; continuing", git_url)
